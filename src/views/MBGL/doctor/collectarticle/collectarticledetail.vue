...
<br/>
    ...
    <div  style="float:left;margin-left:1rem;">参考文献：</div><br/>
        ...
        <div v-if="nexusflg" v-for="item of nexusinfo"  style="height:3rem">
      <div style="float:left;margin-left:1rem;margin-top:1.5rem;margin-bottom:3rem"><span>{{item.style_str}} :</span><a @click="gotoNexusArticle(item.major_id)">{{item.name}}</a></div>
    </div>
  ...
  export default {
    ...
    methods :{
      ...
      gotoQuestions : function(){
        self.common.goToArticleQuestionWithCollected({router: self.$router, articleid : self.articleid});
      },
      //文章反馈
      gotoFeedBack : function(){
        self.common.goToArticleFeedBackWithCollected({router: self.$router, articleid : self.articleid});
      },
      //查看关联文章
      gotoNexusArticle : function(majorid){
        //self.common.goToArticleDetailWithCollected({router: self.$router, articleid : majorid, illid : self.illid});
        self.init(majorid);
      },
      clickBack : function () {
