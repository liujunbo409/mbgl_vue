    ...
    <ul class="aui-list aui-form-list" style="margin-top:0.5rem">
        <div class="aui-border-b">
                <textarea class="aui-font-size-14 aui-padded-10" style="width: 100%;height:auto;border: 0px;
                          ...
                          placeholder="请描述详细的问答反馈内容，我们将尽快为您处理" maxlength="140" v-model="content"></textarea>
                </div>
      </ul>
...
<script>
  ...
  export default {
    ...
    mounted(){
      self.question_id = self.$route.query.question_id;
      self.init();
    },
    data(){
      return {
    ...
    methods :{
      init : function(){
        //获得类型列表
        self.api.user_getFeedBackTypes({type : 'question'}).then((res)=>{
